...
def fit_exponential(dates: np.ndarray, values: np.ndarray,
    ...
    """
    Fit exponential model to data in log space.
    Returns (a, b, r_squared) where model is: y = a * exp(b * (t - t_reference))

    Least squares is done on log(y), so every data point is weighted equally
    in relative (percentage) terms. A linear-space fit would be dominated by
    the largest values when the data spans orders of magnitude, effectively
    ignoring the early data points. R² is also computed in log space.
    """
    t_shifted = dates - t_reference

    log_values = np.log(values)
    b, log_a = np.polyfit(t_shifted, log_values, 1)
    a = np.exp(log_a)

    # R-squared in log space
    predicted = log_a + b * t_shifted
...
openai_data = {
    ...
    'inference_spend_annualized': [
        ...
        # "Spent more than $8 billion on the costs of running its AI models" in 2025
        # (actual, replaces earlier $7B projection)
        DataPoint(datetime(2025, 12, 31), 8e9, 'inference_spend', 'The Information', 'Likely'),
    ],
    ...
    'revenue_annualized': [
        ...
        DataPoint(datetime(2025, 12, 18), 19e9, 'revenue', 'The Information', 'Likely'),
        DataPoint(datetime(2025, 12, 31), 21.4e9, 'revenue', 'The Information', 'Likely'),
        DataPoint(datetime(2026, 2, 28), 25e9, 'revenue', 'The Information', 'Likely'),
    ],
}
...
google_data = {
    ...
    'gemini_mau': [
        ...
        DataPoint(datetime(2026, 2, 4), 750e6, 'mau', 'Q4 2025 Earnings', 'Confident'),
        DataPoint(datetime(2026, 7, 22), 950e6, 'mau', 'Q2 2026 Earnings', 'Confident'),
    ],
    ...
    'api_daily_tokens': [
        ...
        # 10B tokens/minute * 1440 = 14.4T/day ("up from 10 billion last quarter" in Q1 2026 remarks)
        DataPoint(datetime(2026, 2, 4), 14.4e12, 'api_daily_tokens', 'Q1 2026 Earnings (prior quarter)', 'Confident'),
        # 16B tokens/minute * 1440 = 23.04T/day
        DataPoint(datetime(2026, 4, 29), 23.04e12, 'api_daily_tokens', 'Q1 2026 Earnings', 'Confident'),
    ],
}
...
def build_gemini_api_tokens():
    """
    Build Gemini API daily token estimates from direct API token measurements.

    With a single data point, growth rate is applied later in model fitting
    using the averaged industry growth rate.
    """
    estimates = [(dp.date, dp.value, 'api_direct') for dp in google_data['api_daily_tokens']]
    ...
    for _, row in df.iterrows():
        print(f"  {row['date'].strftime('%Y-%m-%d')}: {row['daily_tokens']/1e12:.2f}T/day (direct measurement)")
    if len(df) == 1:
        print(f"  NOTE: Single point - will use averaged industry growth rate for trendline")
    print()
...
anthropic_data = {
    # Note: build_anthropic_daily_tokens() anchors on the FIRST entry (Jul 2025)
    'inference_spend_annualized': [
        DataPoint(datetime(2025, 7, 31), 2e9, 'inference_spend', 'The Information/Morgan Stanley', 'Likely'),
        # 2025 full-year inference costs (of ~$4.1B total model costs)
        DataPoint(datetime(2025, 12, 31), 2.7e9, 'inference_spend', 'The Information', 'Likely'),
    ],
    'revenue_annualized': [
        ...
        DataPoint(datetime(2025, 12, 31), 9e9, 'revenue', 'Bloomberg', 'Likely'),
        DataPoint(datetime(2026, 2, 12), 14e9, 'revenue', 'Anthropic (Series G)', 'Confident'),
        DataPoint(datetime(2026, 3, 3), 19e9, 'revenue', 'X post (Apoorv)', 'Confident'),
        DataPoint(datetime(2026, 4, 6), 30e9, 'revenue', 'Anthropic', 'Confident'),
        DataPoint(datetime(2026, 5, 15), 47e9, 'revenue', 'Anthropic (Series H)', 'Likely'),
    ],
}
...
openai_inference_spend = {
    ...
    2025: 8e9,    # Actual (The Information)
}
...
xai_data = {
    ...
    # Grok MAU (audited, from SpaceX S-1 filed May 2026). Not used by the
    # revenue-based token model; kept for reference.
    'mau': [
        DataPoint(datetime(2025, 12, 31), 35e6, 'mau', 'SpaceX S-1', 'Confident'),
        DataPoint(datetime(2026, 3, 31), 117e6, 'mau', 'SpaceX S-1', 'Confident'),
    ],
    'revenue_annualized': [
        ...
        DataPoint(datetime(2025, 9, 30), 428e6, 'revenue', 'Bloomberg (Q3 annualized)', 'Likely'),
    ],
}
...
def create_revenue_chart():
    """
    Create a chart estimating revenue directly for OpenAI and Anthropic.

    Unlike the token charts (which use revenue as a proxy for token production),
    this plots annualized revenue itself, using the Epoch AI revenue dataset
    (ai_companies_revenue.csv) rather than the hardcoded data points above.
    An exponential model is fit per company and extrapolated forward.
    """
    df = pd.read_csv('ai_companies_revenue.csv')
    df['Date'] = pd.to_datetime(df['Date'])

    # Full-company annualized revenue reports only (excludes product/division
    # figures like ChatGPT-only revenue, and rows with no annualized number)
    df = df[(df['Scope'] == 'Full company') & df['Annualized revenue (USD)'].notna()]

    styles = {
        'OpenAI': {'color': '#10a37f', 'marker': 'o'},
        'Anthropic': {'color': '#c1622b', 'marker': 's'},
    }

    fig, ax = plt.subplots(figsize=(7, 4.1))

    t_plot = np.linspace(2023.0, PLOT_END_DATE, 300)

    print("\nRevenue estimates (direct, from Epoch data):")
    for company, style in styles.items():
        sub = df[df['Company'] == company].sort_values('Date')
        dates = np.array([date_to_decimal(d) for d in sub['Date']])
        values = sub['Annualized revenue (USD)'].values.astype(float)

        a, b, r_squared = fit_exponential(dates, values)

        ax.scatter(dates, values / 1e9, s=60, color=style['color'],
                   marker=style['marker'], alpha=0.7, zorder=5, label=f'{company} (reported)')

        y_fit = exponential_model(t_plot - 2024, a, b)
        ax.plot(t_plot, y_fit / 1e9, linestyle='--', color=style['color'],
                alpha=0.8, linewidth=2, label=f'{company} fit ({np.exp(b):.1f}x/year)')

        # Direct label at the latest data point
        ax.annotate(company, (dates[-1], values[-1] / 1e9),
                    textcoords='offset points', xytext=(8, -4),
                    fontsize=10, fontweight='bold', color=style['color'])

        latest = sub.iloc[-1]
        print(f"  {company}: {len(sub)} data points, growth {np.exp(b):.2f}x/year, "
              f"R²={r_squared:.3f}, latest ${latest['Annualized revenue (USD)']/1e9:.1f}B "
              f"({latest['Date'].strftime('%b %Y')})")

    ax.set_yscale('log')
    ax.set_xlabel('Year', fontsize=12)
    ax.set_ylabel('Annualized Revenue ($ Billions)', fontsize=12)
    ax.set_title('Revenue - Direct Estimates (OpenAI & Anthropic)', fontsize=14, fontweight='bold')

    ax.legend(loc='upper left', fontsize=9)
    ax.grid(True, alpha=0.3, which='both')
    ax.set_xlim(2023.0, PLOT_END_DATE)

    plt.tight_layout()
    filepath = os.path.join(PLOTS_FOLDER, 'revenue.png')
    plt.savefig(filepath, dpi=150, bbox_inches='tight')
    plt.close(fig)
    print(f"  Saved {filepath}")
...
""")
    ...
    # Create direct revenue chart (OpenAI & Anthropic)
