...
def main():
            ...
            for af in ar_forms:
                for hf in he_forms:
                    if hf in ar2he_cog.get(af, set()):
                        ...
                        if "direct_cognate_ar→he" not in d["layers"]:
    ...
    for he_norm, ar_norms in he2ar_cog.items():
        for ar_norm in ar_norms:
            if (ar_norm, he_norm) in false_pos:
                continue
            d = _ensure_pair(ar_norm, he_norm)
            if "direct_cognate_he→ar" not in d["layers"]:
    ...
    print(f"  {len(results)} cognate pairs written")
