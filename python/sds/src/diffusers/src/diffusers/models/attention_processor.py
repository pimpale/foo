    ...
    def __call__(
        ...
        if(encoder_hidden_states is not None):
            if(type(encoder_hidden_states) == dict):
                input_dict = encoder_hidden_states
                ca_mask = input_dict["mask"]
                encoder_hidden_states = input_dict["embs"]
                mod_flag = True
        ...
        # TODO: add support for attn.scale when we move to Torch 2.1
        if(mod_flag):
            spatial_dim = int(hidden_states.shape[1] ** (1/2))
            ca_dim = encoder_hidden_states.shape[1]
            interp_mask = F.interpolate(ca_mask, size=(spatial_dim,spatial_dim), mode='bilinear')
            interp_mask = interp_mask[0,0]
            interp_mask = interp_mask.flatten().unsqueeze(-1)-1
            # print("interp", interp_mask.shape)

            attention_mask = torch.zeros((batch_size, attn.heads, interp_mask.shape[0], ca_dim), dtype=torch.float16).to(torch.device("cuda"))
            # print(attention_mask.shape)
            attention_mask[4:,:,:,6:10] += torch.where(interp_mask >= -0.99, -10000, interp_mask).type(torch.float16)
            attention_mask[4:,:,:,3:6] += torch.where(interp_mask >= -0.99, interp_mask, -10000).type(torch.float16)
            ...
            with torch.no_grad():
                ...
                input_dict["attn_masks"].append(attn_weight)
