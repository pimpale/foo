...
for i, (family, release_date) in enumerate(families_release_dates):
    ...
    xpoints = base_llm_benchmark_eval[
...
ax.set_title(f"FLOPs_opt vs deprogressed PC-1")
ax.set_xlabel("log10 FLOPs_opt (1E21)")
...
ax[0].plot(
    xspace,
    y_line,
    label=f"Expected log10 FLOPs_opt",
)
...
ax.set_title(f"MMLU vs FLOPs_opt")
ax.set_xlabel("log10 FLOPs_opt (1E21)")
ax.set_ylabel("MMLU")
norm = mcolors.Normalize(vmin=min(release_dates), vmax=max(release_dates))
cmap = plt.get_cmap("viridis")
...
ax.set_title(f"Linear PC-1 vs FLOPs_opt")
ax.set_xlabel("log10 FLOPs_opt (1E21)")
