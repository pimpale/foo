...
log_bins = np.geomspace(
    min(all_task_ttfs["minutes"]), max(all_task_ttfs["minutes"]), 10
)
...
# compute q(x) for all tasks
qx = stats.lognorm.pdf(all_task_ttfs["minutes"], s=q_std, scale=np.exp(q_mu)) / q_z
...
def plot_task_success_rate_vs_elo(
    title, ax, tasks, use_lognormal=True, threshold_elo=1200
):
    ...
    ax.text(
        0.1,
        0.95,
        f"[train] Sigmoid MSE: {train_mse_sigmoid:.2e}",
        transform=ax.transAxes,
    )
    ax.text(
        0.1, 0.9, f"[test] Sigmoid MSE: {test_mse_sigmoid:.2e}", transform=ax.transAxes
    )
    ...
    if use_lognormal:
        ...
        ax.text(
            0.1,
            0.85,
            f"[train] Lognormal MSE: {train_mse_lognormal:.2e}",
            transform=ax.transAxes,
        )
        ax.text(
            0.1,
            0.8,
            f"[test] Lognormal MSE: {test_mse_lognormal:.2e}",
            transform=ax.transAxes,
        )
...
plot_task_success_rate_vs_elo("Unweighted Tasks", ax1, tasks, use_lognormal=True)

fig.legend(loc="upper center", bbox_to_anchor=(0.5, 1.05), ncol=4)

plot_task_success_rate_vs_elo(
    "Weighted Tasks (Importance Sampled)", ax2, tasks_weighted, use_lognormal=False
)
